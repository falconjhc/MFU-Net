...
class SDNetImageCallback(BaseSaveImage):
    def __init__(self, conf, sdnet, data_gen_lb, data_gen_ul, mask_gen, img_channels, mask_channels):
        '''
    ...
    def on_epoch_end(self, epoch=None, logs=None):
        ...
        mks = lb_image_mask_pack[:,:,:,self.img_channels:]
        lb_images = []
        lb_images.append(ims)
        lb_images.append(mks)
    ...
    def plot_segmentations(self, lb_images, ul_images, epoch):
        ...
        for i in range(x.shape[0]):
            ...
            rows += [np.concatenate([x[i, :, :, current_select]] + y_list + m_list, axis=1)]

        im_plot = np.concatenate(rows, axis=0)
        imsave(self.segm_folder + '/segmentations_epoch_%d.png' % (epoch), im_plot)
        # harric modified
    ...
    def plot_reconstructions(self, lb_images, ul_images, epoch):
        ...
        rows = [np.concatenate([x[i, :, :, current_select], y[i, :, :, current_select], y_random[i, :, :, current_select], y_s0[i, :, :, current_select]] +
                               [self.reconstructor.predict([self._get_s0chn(k, s), z])[i, :, :, current_select] for k in
                                range(s.shape[-1] - 1)] +
                               [y_0z[i, :, :, current_select], y_00[i, :, :, current_select]], axis=1) for i in range(x.shape[0])]
        ...
        imsave(self.rec_folder + '/rec_epoch_%d.png' % epoch, im_plot)
        # harric modified
    ...
    def plot_image_interpolation(self, lb_images, ul_images, epoch):
        ...
        for i in range(0, 4, 2):
            ...
            imsave(self.interp_folder + '/interpolation1_epoch_%d.png' % epoch, self._interpolate(s1, z1, z2, current_selected))
            imsave(self.interp_folder + '/interpolation2_epoch_%d.png' % epoch, self._interpolate(s2, z2, z1, current_selected))

    def _interpolate(self, s, z1, z2, current_selected):
        row1, row2 = [], []
        for w1, w2 in zip(np.arange(0, 1, 0.1), np.arange(1, 0, -0.1)):
            sum = w1 * z1 + w2 * z2
            rec = self.reconstructor.predict([s, sum])[0, :, :, current_selected]
            if w1 < 0.5:
                row1.append(rec)
            else:
                row2.append(rec)
